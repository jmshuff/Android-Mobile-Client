...
public class PatientsDAO {
    ...
    public List<String> getPatientUUIDs(String houseHoldValue) throws DAOException {
        ...
        db.beginTransaction();
        try {
            Cursor cursor = db.rawQuery("SELECT patientuuid FROM tbl_patient_attribute where value = ? " +
                    "AND (sync='0' OR sync='TRUE') COLLATE NOCASE", new String[]{houseHoldValue});
...
//                  middle_name = cursor.getString(cursor.getColumnIndexOrThrow("middle_name"));
    ...
    public boolean getPatientAttributee(String patientUUID_fam_registration) {
        boolean exist = false;
